  ...
  StudentEnrollment.findOne({
    ...
    .then(async data => {
      ...
      if (data) {
        ...
        res.status(200).json(success("Student Enrollment details fetched successfully!", academic));
      
        
      }else{
        res.status(400).json(errorResponse(err + "UP Error retrieving Student Enrollment details with id=" + id, 400));
      }
    
      
    }).catch(err => {
        res.status(400).json(errorResponse(err + " Error retrieving Student Enrollment details with id=" + id, 400));
    });
};
  ...
  await InstituteProgramme.findOne({
    ...
    .then(async (instprog) => {     
      await StudentEnrollment.update({
        ...
        current_class: req.body.class_id,
        current_semester: req.body.current_semester,
        subject_id: req.body.subject_id,
      }, {where: {user_id: req.user.id } })
    .then(async data => {      
      res.status(200).json(success("Student Enrollment details updated successfully!", data));
    })     
  }).catch(err => {
    res.status(400).json(errorResponse(err + " Error updating Student Enrollment details with id=" + id, 400));
});    
