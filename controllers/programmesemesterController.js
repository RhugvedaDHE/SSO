...
exports.create = async (req, res) => {
    ...
    ProgrammeSemester.create(programmeSemester)
      ...
      .catch(err => {
        res.status(500).send({
          message:
            err.message || "Some error occurred while creating the ProgrammeSemester."
        });
    });
};
...
exports.deleteAll = (req, res) => {
    ...
    .then(nums => {
      res.send({ message: `${nums} ProgrammeSemester were deleted successfully!` });
    })
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while removing all ProgrammeSemester."
      });
    });
};
